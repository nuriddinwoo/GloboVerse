...
Verified grants first flush active countdown usage, then use a crash-recoverable journal and stable server verification IDs. The exact newly applied grant is merged into the live countdown and that merged balance is persisted before store completion, so elapsed time during local grant writes is preserved and duplicate delivery cannot add an hour twice. Android consumables are consumed only after a verified or definitively rejected response. Transient and pending checks remain unfinished for retry. An authenticated snapshot is reconciled at startup, after restore, on foreground resume, and periodically; only newer revisions can replace VIP state, including clearing revoked access. See the full request/response, idempotency, migration, and backend validation contract in [docs/billing_api.md](docs/billing_api.md).
