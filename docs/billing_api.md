...
Purchase updates and settings entitlement writes are serialized. After a verified response, the active session first persists elapsed usage; if that flush fails, the purchase stays unfinished for retry. `SettingsService` then writes a recovery journal containing the stable server `verificationId`, authoritative snapshot revision, and exact target local state; it persists that state before recording the ID as applied. The app merges only the exact newly applied trusted delta into the live countdown, preserving seconds consumed while the journal completes, and durably writes the merged value before finishing the store item. A post-grant flush failure is also retryable. Re-delivery reports no new delta and cannot add another hour, while a newer attached snapshot can still revoke or refresh VIP. A journal left by an interrupted write is completed on the next startup.
